...
public final class TagCellLayout: UICollectionViewFlowLayout {
    ...
    var minimumInterItemSpace: CGFloat = 0
    var minimumInterLineSpace: CGFloat = 0
	...
	override public var collectionViewContentSize: CGSize {
		...
		let height = layoutInfoList
			...
			.reduce(0, { $0 + $1.layoutAttribute.frame.height + minimumInterLineSpace }) + inset.top + inset.bottom
...
private extension TagCellLayout {
	...
	func tagCellLayoutInfo(tagIndex: Int, tagSize: CGSize) -> LayoutInfo {
		...
		tagFrame.origin.x += (tagFrame.origin.x == 0) ? inset.left : minimumInterItemSpace
        tagFrame.origin.y = (tagFrame.origin.y == 0) ? inset.top : tagFrame.origin.y
        
		// if next tag goes out of screen then move it to next row
		if shouldMoveTagToNextRow(tagWidth: tagSize.width) {
			tagFrame.origin.x = inset.left
			tagFrame.origin.y += (currentTagFrame.height + minimumInterLineSpace)
	...
	func shouldMoveTagToNextRow(tagWidth: CGFloat) -> Bool {
		return ((currentTagFrame.origin.x + tagWidth) > (collectionViewWidth - (inset.left + inset.right)))
	}
